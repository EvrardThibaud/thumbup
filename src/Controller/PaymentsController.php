...
final class PaymentsController extends AbstractController
{
    ...
    public function checkout(Request $req, OrderRepository $ordersRepo): Response
    {
        ...
        if (!$ids) { $this->addFlash('warning','Your cart is empty.'); return $this->redirectToRoute('app_payments_index'); }

        $orders = []; $totalCents = 0;
        foreach ($ids as $id) {
            $o = $ordersRepo->find($id);
            if (!$o instanceof Order) continue;
            if ($o->getClient() !== $client || $o->isPaid() || !in_array($o->getStatus(), [OrderStatus::DELIVERED, OrderStatus::REVISION, OrderStatus::FINISHED], true)) continue;
            $orders[] = $o; $totalCents += (int)$o->getPrice();
        }
        if (!$orders) { $this->addFlash('warning','No eligible orders to pay.'); return $this->redirectToRoute('app_payments_index'); }
    ...
    public function paypalCreate(Request $req, OrderRepository $ordersRepo, EntityManagerInterface $em, PaymentRepository $payRepo): Response
    {
        $this->denyAccessUnlessGranted('ROLE_CLIENT');
        $client = $this->getUser()?->getClient(); if (!$client instanceof Client) { throw $this->createAccessDeniedException(); }
        $ids = (array)$req->getSession()->get('checkout.order_ids', []);
        if (!$ids) return $this->redirectToRoute('app_payments_checkout');
        ...
        if ($totalCents<=0) { $this->addFlash('warning','No eligible orders to pay.'); return $this->redirectToRoute('app_payments_checkout'); }
        ...
        $r->body = [
            'intent'=>'CAPTURE',
            'purchase_units'=> [[
                'amount'=> ['currency_code'=>'EUR','value'=> number_format($totalCents/100,2,'.','')],
                'description'=> sprintf('ThumbUp orders: %s', implode(',',$ids)),
                'custom_id'=> implode(',',$ids),
            ]],
            'application_context'=>[
                'brand_name'=>'ThumbUp',
                'landing_page'=>'LOGIN',
                'user_action'=>'PAY_NOW',
                'return_url'=> $this->generateUrl('app_payments_checkout_paypal_return', [], \Symfony\Component\Routing\Generator\UrlGeneratorInterface::ABSOLUTE_URL),
                'cancel_url'=> $this->generateUrl('app_payments_checkout_paypal_cancel', [], \Symfony\Component\Routing\Generator\UrlGeneratorInterface::ABSOLUTE_URL),
            ],
        ];
        $res = $pp->execute($r);
        ...
        $approveUrl = null;
        foreach ($res->result->links ?? [] as $ln) if (($ln->rel ?? '')==='approve') { $approveUrl = $ln->href ?? null; break; }
        if (!$approveUrl) { $this->addFlash('danger','PayPal error: approval link not found.'); return $this->redirectToRoute('app_payments_checkout'); }

        return new RedirectResponse($approveUrl);
    }

    #[Route('/checkout/paypal/return', name: 'app_payments_checkout_paypal_return', methods: ['GET'])]
    public function paypalReturn(Request $req, OrderRepository $ordersRepo, PaymentRepository $payRepo, EntityManagerInterface $em): Response
    {
        $this->denyAccessUnlessGranted('ROLE_CLIENT');
        $token = (string)$req->query->get('token'); if (!$token) { $this->addFlash('danger','Missing PayPal token.'); return $this->redirectToRoute('app_payments_checkout'); }
